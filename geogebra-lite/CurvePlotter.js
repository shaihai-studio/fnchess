class CurvePlotter {
  constructor(curve, tMin, tMax, view, gp, calcLabelPos, moveToAllowed) {
    ...
    const minSamplePoints = Math.max(1200, Math.floor(width * 4));
    const maxParamStep = Math.abs(tMax - tMin) / minSamplePoints;
    this.curveSegmentPlotter = new CurveSegmentPlotter(curve, tMin, tMax, 0, maxParamStep, view, gp, calcLabelPos, moveToAllowed, true);
